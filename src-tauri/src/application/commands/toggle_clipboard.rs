...
mod tests {
    ...
    #[test]
    fn test_toggle_clipboard_enables_observer() {
    ...
    #[test]
    fn test_toggle_clipboard_disables_observer() {
        let bus = make_command_bus();
    ...
    #[test]
    fn test_toggle_clipboard_updates_config() {
        let bus = make_command_bus();
        let _ = bus.handle_toggle_clipboard(true);
        let config = bus.config_store().get_config().unwrap();
        assert!(config.clipboard_monitoring);

        let _ = bus.handle_toggle_clipboard(false);
        let config = bus.config_store().get_config().unwrap();
        assert!(!config.clipboard_monitoring);
    }
}
