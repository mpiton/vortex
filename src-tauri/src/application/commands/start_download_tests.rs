...
fn make_command_bus(
    ...
    make_command_bus_with_plugin_loader(http_client, Arc::new(MockPluginLoader))
}

fn make_command_bus_with_plugin_loader(
    http_client: Arc<dyn HttpClient>,
...
async fn test_start_download_persists_and_emits_event() {
    ...
    let (bus, repo, event_bus) = make_command_bus(Arc::new(MockHttpClient::with_response(resp)));
...
async fn hoster_without_filename_never_uses_the_generic_head_probe() {
    ...
    let (bus, repo, _) = make_command_bus_with_plugin_loader(
