...
- Plugin configuration UI dynamique (PRD-v2 P0.15, task 15): plugins déclarant un bloc `[config]` dans leur `plugin.toml` exposent désormais leur schéma au runtime. Backend ajoute `ConfigField` / `ConfigFieldType` / `PluginConfigSchema` dans `domain/model/plugin.rs` (validation typée, options enum, bornes `min`/`max`, regex via un matcher std-only — pas d'import externe en domaine), un port `PluginConfigStore` (`get_values` / `set_value` / `list_all` / `delete_all`) implémenté par `SqlitePluginConfigRepo` adossé à la nouvelle table `plugin_configs (plugin_name, key, value)` (migration `m20260425_000005_create_plugin_configs`, clé primaire composite). Le parser de manifest (`adapters/driven/plugin/manifest.rs`) extrait désormais `type`, `default`, `options`, `description`, `min`, `max`, `regex` en plus des défauts existants. CQRS gagne `UpdatePluginConfigCommand` (valide contre le schéma, persiste, met à jour la map runtime du loader) et `GetPluginConfigQuery` (renvoie schéma + valeurs persistées avec fallback vers les défauts du manifest). `PluginLoader` est étendu avec `get_manifest()` et `set_runtime_config()` ; `ExtismPluginLoader` les implémente en lisant `PluginRegistry` et en écrivant dans `SharedHostResources::plugin_configs`, donc les appels `get_config(key)` côté plugin WASM voient la nouvelle valeur sans reload. Au démarrage, `lib.rs` rejoue les configs persistées sur la map in-memory avant que les plugins ne soient chargés. Frontend ajoute deux composants : `PluginConfigField.tsx` (dispatcher renderer : `string` → input texte, `boolean` → switch shadcn, `integer`/`float` → input numérique avec bornes, `url` → input url, `enum` (et `string` avec options) → select shadcn) et `PluginConfigDialog.tsx` (charge le schéma via `useQuery`, valide chaque champ côté UI avant l'envoi, persiste les valeurs modifiées séquentiellement, invalide la query au succès). `PluginsView` interroge `plugin_config_get` pour chaque plugin installé pour décider si le bouton *Configure* (icône Settings, à côté du menu *More*) doit s'afficher : un plugin sans `[config]` n'expose aucun bouton. Nouvelles commandes IPC `plugin_config_get(name) → PluginConfigView` et `plugin_config_update(name, key, value)`. i18n (en/fr) : `plugins.action.configure`, `plugins.config.{title,description,loading,error,noFields,toast.{saveSuccess,validationFailed}}`. (task 15)
