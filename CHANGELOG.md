...
- SoundCloud, Vimeo, and Gallery WASM plugins (Task 25)
  ...
  - **SoundCloud plugin**: `/resolve` API client (api-v2.soundcloud.com) with tagged enum `ResolveResponse` (Track / Playlist / User / Unknown), `classify_url` router covering `soundcloud.com`, `m.soundcloud.com`, `on.soundcloud.com` plus `sets/`, `likes`, `tracks`, `albums` paths, artwork upgrade from `-large` to `-t500x500`, `client_id` forwarded via host `get_config`, and 38 native unit tests
  - **Vimeo plugin**: oEmbed JSON client (`vimeo.com/api/oembed.json`) for metadata + player config client (`player.vimeo.com/video/<id>/config`) for quality variants (progressive MP4 + HLS), balanced-brace HTML fallback to extract `window.playerConfig` from embedded pages, `pick_variant_for_quality` with `2K → 1440` / `4K → 2160` mapping, `filter_audio_only` preserving HLS, public / private-hash / showcase URL classification, and 43 native unit tests
  - **Gallery plugin**: 3 provider backends with dedicated JSON shapes — Imgur album API v3 (Authorization: Client-ID), Reddit submission JSON (both native `is_gallery` + single-image preview fallback) with `&amp;` unescaping and deterministic URL-sorted output, Flickr `flickr.photosets.getPhotos` (handles both numeric and string `width_o`/`height_o`); generic `<img src>` HTML fallback behind a separate `extract_generic` export to avoid claiming ownership of every HTTP page; `has_non_http_scheme` guard blocks `data:`/`javascript:`/`mailto:` from resolution; post-processing pipeline: `dedupe_links` → `filter_by_min_resolution` (preserves unknown dimensions) → `auto_name` (zero-padded `<provider>_<album>_<idx>.<ext>` with album id sanitisation), and 45 native unit tests
  ...
  - Release WASM binaries: SoundCloud 250 KB, Vimeo 1.1 MB, Gallery 1.1 MB (all stripped with LTO + `opt-level = "z"`)
