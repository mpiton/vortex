...
function renderDialog(
  overrides: Partial<Parameters<typeof ClearDownloadsDialog>[0]> = {},
) {
  const props = {
    open: true,
    onOpenChange: vi.fn(),
...
describe('ClearDownloadsDialog', () => {
  it('renders the completed title with the provided count', () => {
    ...
    expect(
      screen.getByText(/Clear 3 completed downloads\?/i),
    ).toBeInTheDocument();
  });
  ...
  it('renders the failed title when targetState is error', () => {
    ...
    expect(
      screen.getByText(/Clear 2 failed downloads\?/i),
    ).toBeInTheDocument();
  });
  ...
  it('reveals the warning panel when the checkbox is checked', async () => {
    ...
    renderDialog();
    await user.click(
      screen.getByRole('checkbox', { name: /also delete files from disk/i }),
    );
