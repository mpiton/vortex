...
describe("i18n translation files", () => {
  ...
  it("should have settings tabs for all 7 sections", () => {
    const tabsEn = (en.settings as Record<string, unknown>).tabs as Record<string, string>;
    expect(Object.keys(tabsEn)).toHaveLength(7);
    expect(tabsEn.shortcuts).toBeDefined();
  });

  it("should have appearance keys for theme, accent, compact and language", () => {
    const appearance = (en.settings as Record<string, unknown>).appearance as Record<
      string,
      string
    >;
