...
describe("usePackageByExternalId", () => {
  it("calls invoke with externalId arg when defined", async () => {
    ...
    const { result } = renderHook(() => usePackageByExternalId("youtube:playlist:abc"), {
      wrapper: wrapper(),
    });
    await waitFor(() => expect(result.current.isLoading).toBe(false));

    expect(mockInvoke).toHaveBeenCalledWith("package_find_by_external_id", {
  ...
  it("does not call invoke when externalId is undefined", async () => {
    const { result } = renderHook(() => usePackageByExternalId(undefined), { wrapper: wrapper() });
  ...
  it("returns null when no package matches", async () => {
    ...
    const { result } = renderHook(() => usePackageByExternalId("youtube:playlist:xyz"), {
      wrapper: wrapper(),
    });
    await waitFor(() => expect(result.current.isLoading).toBe(false));
