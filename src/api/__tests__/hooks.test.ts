...
describe("useTauriQuery", () => {
  ...
  it("should call tauriInvoke with command and args", async () => {
    ...
    const { result } = renderHook(() => useTauriQuery("get_download", { id: "1" }), {
      wrapper: makeWrapper(),
    });
  ...
  it("should expose error on failure", async () => {
    ...
    const { result } = renderHook(() => useTauriQuery("get_download", { id: "99" }), {
      wrapper: makeWrapper(),
    });
  ...
  it("should call tauriInvoke without args when none provided", async () => {
    ...
    const { result } = renderHook(() => useTauriQuery("list_downloads"), {
      wrapper: makeWrapper(),
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
...
describe("useTauriMutation", () => {
  ...
  it("should call tauriInvoke with command on mutate", async () => {
    ...
    const { result } = renderHook(() => useTauriMutation("download_pause"), {
      wrapper: makeWrapper(),
    });
    await act(async () => {
  ...
  it("should NOT surface toast.error when silentError is true", async () => {
    ...
    const { result } = renderHook(() => useTauriMutation("download_pause", { silentError: true }), {
      wrapper: makeWrapper(),
    });
    await act(async () => {
      result.current.mutate({ id: "1" } as Record<string, unknown>);
